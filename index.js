...
// add gmail
if (msg.body.startsWith('.setgmailprice')) {
const groupChat = await msg.getChat();
const botChatObj = groupChat.participants.find(chatObj => chatObj.id.user === client.info.wid.user);
  if (botChatObj.isAdmin){
    const give = msg.body;
...
if (commandParts.length !== 3 || commandParts[0] !== '.setgmailprice') {
console.log('Invalid command format.');
...
//show all gmails
     ...
     const give = msg.body;
     // Example: Set the targetCountry based on user input or command
const fullCommand = give;
const commandParts = fullCommand.split(' ');
...
const targetCountry = commandParts[1]; // Assuming the target country is the second part of the command
...
if (targetCountry === 'all') {
  if (Object.keys(stockByCountry).length === 0) {
    ...
    for (const country in stockByCountry) {
      ...
      const priceInfo = priceData.find(item => item.country === country);
      const price = priceInfo ? priceInfo.price : 'Price not available';
      console.log(`Country: ${country}, Stock: ${stock}, Price: ${price}`);
      msg.reply(`*Country =>* ${country}\n*Stock =>* ${stock-1}\n*Price =>* ${price} USD (per gmail)\n..............\nFor buying type *.gmailbuy country quantity*`)
    }
  }
} else {
  if (existingData.length === 0) {
    ...
    const priceInfo = priceData.find(item => item.country === targetCountry);
    const price = priceInfo ? priceInfo.price : 'Price not available';
...
//for buying gmails
if (msg.body.startsWith('.gmailbuy')) {
  const givy = msg.body;
  const senderNumbery = msg.from;
  const inputy = senderNumbery;
  const phoneNumberRegexy = /(\d+)/;
  const match = inputy.match(phoneNumberRegexy);
    const phoneNumberrrrr = match[1];
    // Example: Set the command, country, and quantity
const fullCommanddd = givy;
const commandParts = fullCommanddd.split(' ');

if (commandParts.length !== 3 || commandParts[0] !== '.gmailbuy' || commandParts[2] < 1) {
  console.log('Invalid command format.');
  msg.reply('The correct command format is this *.gmailbuy country quantity*');
  
}

const targetCountry = commandParts[1];
const quantity = parseInt(commandParts[2]);

// Load balance data from the JSON file
const balanceFilePath = path.join(__dirname, 'phone_data.json');
let balanceData = [];

try {
  if (fs.existsSync(balanceFilePath)) {
    const balanceFileData = fs.readFileSync(balanceFilePath, 'utf8');
    balanceData = JSON.parse(balanceFileData);
  }
} catch (error) {
  console.log('Error reading or parsing the balance file:', error.message);
...
// Load gmail data from the JSON file
const gmailFilePath = path.join(__dirname, 'gmail_list.json');
let gmailData = [];

try {
  if (fs.existsSync(gmailFilePath)) {
    const gmailFileData = fs.readFileSync(gmailFilePath, 'utf8');
    gmailData = JSON.parse(gmailFileData);
  }
} catch (error) {
  console.log('Error reading or parsing the gmail file:', error.message);
  
}

// Find balance for the provided phone number
//const phoneNumberrrrr = '923497985307'; // Replace with your provided phone number
const userBalance = balanceData.find(item => item.phoneNumber === phoneNumberrrrr)?.balance || 0;

// Find price for the provided country
const countryPrice = priceData.find(item => item.country === targetCountry)?.price;
if (!countryPrice) {
  console.log(`Price not available for country: ${targetCountry}`);
  msg.reply(`Price not available for country: ${targetCountry}`);
  
}

// Calculate total cost
const totalCost = countryPrice * quantity;

// Check if user has enough balance
if (userBalance < totalCost) {
  console.log(`Insufficient balance. Your balance: ${userBalance}, Required: ${totalCost}`);
  msg.reply(`Insufficient balance. Your balance: *${userBalance}*\nFor Deposit type *.deposit*`);
  
}

// Filter and get gmails for the corresponding country
const filteredGmails = gmailData.filter(item => item.country === targetCountry);

//10 9 Check if there are enough gmails available
if (filteredGmails.length <= quantity || totalCost > userBalance || quantity < 1) {
  console.log(`Not enough gmails available for ${targetCountry}`);
  msg.reply(`Not enough gmails available for *${targetCountry}*`);
}else{
// Update user's balance
const updatedBalance = userBalance - totalCost;
const userIndex = balanceData.findIndex(item => item.phoneNumber === phoneNumberrrrr);
if (userIndex !== -1) {
  balanceData[userIndex].balance = updatedBalance;
}

// Write the updated balance data back to the phone_data.json file
try {
  fs.writeFileSync(balanceFilePath, JSON.stringify(balanceData, null, 2));
  console.log(`Balance updated. Remaining balance: ${updatedBalance}`);
} catch (error) {
  console.log('Error updating balance file:', error.message);
}

// Display and remove random gmails
console.log(`You have successfully bought ${quantity} gmails from ${targetCountry}:`);
msg.reply(`You have successfully bought *${quantity}* gmails from *${targetCountry}*\nBelow are the gmails`)
const purchasedGmails = [];

for (let i = 0; i < quantity; i++) {
  if (filteredGmails.length === 0) {
    console.log('No more gmails available.');
    msg.reply('No more gmails available.')
    break;
  }

  const randomIndex = Math.floor(Math.random() * filteredGmails.length);
  const randomGmail = filteredGmails[randomIndex];
  console.log(`Country: ${randomGmail.country}, Email: ${randomGmail.email}, Password: ${randomGmail.password}`);
  msg.reply(`\n*Country =>* ${randomGmail.country}\n*Email =>* ${randomGmail.email}\n*Password =>* ${randomGmail.password}\n\nIf you need otp for this gmail click on this link => https://api.whatsapp.com/send?phone=+923497985307&text=I_need_OTP_for_this_gmail(${randomGmail.email})and_password(${randomGmail.password})`);
  const groupName = 'Demo';
  // Replace 'Your message here' with the actual message you want to send
  const message = `A Buying Request of *${targetCountry}* gmails  has been placed`;
  // Find the group chat by name
  client.getChats().then((chats) => {
      const groupChat = chats.find((chat) => chat.isGroup && chat.name === groupName);
      if (groupChat) {
  ...
  purchasedGmails.push(randomGmail);
  filteredGmails.splice(randomIndex, 1); // Remove the selected gmail from the list
}

// Remove purchased gmails from gmailData and update the file
const updatedGmailData = gmailData.filter(item => !purchasedGmails.includes(item));

try {
  fs.writeFileSync(gmailFilePath, JSON.stringify(updatedGmailData, null, 2));
  console.log('Gmails updated and saved.');
} catch (error) {
  console.log('Error updating gmail file:', error.message);
}
}
